...
export class Client extends ClientBase {
     ...
     * @return Success
     */
    getAll(version: string): Promise<CarListVm> {
    ...
    protected processGetAll(response: Response): Promise<CarListVm> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
                let result200: any = null;
                result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as CarListVm;
                return result200;
            });
        } else if (status === 401) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<CarListVm>(null as any);
    }
     ...
     * @return Success
     */
    get(id: string, version: string): Promise<CarDetailsVm> {
    ...
    protected processGet(response: Response): Promise<CarDetailsVm> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
                let result200: any = null;
                result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as CarDetailsVm;
                return result200;
            });
        } else if (status === 401) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<CarDetailsVm>(null as any);
    }
